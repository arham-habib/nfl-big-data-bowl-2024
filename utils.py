...
def tackle_percentage_contribution_per_frame(frame_data:pd.DataFrame, weights: pd.DataFrame=None, x_step: int=1, y_step: int=1)->dict:
    ...
    if weights: 
        ...
        baseline_area = voronoi_area(squares).get(ball_carrier, 0) # the baseline area starts at 0 if the player has ownership over 0 voronoi spaces (this catches some common errors)
